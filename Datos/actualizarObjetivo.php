...
//echo $idObj." ".$idPOA;
$consulta = $conectar->prepare("CALL pa_modificar_objetivos_institucionales(?,?,?,?,?,?)");
$consulta->bind_param('isssii',$idObj, $def, $area, $res, $tipArea, $idPOA);
$resultado = $consulta->execute();

if ($resultado == 1) {
    echo '<div id="resultado" class="alert alert-success">
        se ha Modificado un Objetivo Institucional
         
         </div>';
} else {
    echo '<div id="resultado" class="alert alert-danger">
        No se Modifico ningun elemento 
