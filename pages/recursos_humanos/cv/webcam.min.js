...
var Webcam = {version: "1.0.0", protocol: location.protocol.match(/https/i) ? "https" : "http", swfURL: "", loaded: false, live: false, userMedia: true, params: {width: 0, height: 0, dest_width: 0, dest_height: 0, image_format: "jpeg", jpeg_quality: 90, force_flash: false}, hooks: {load: null, live: null, uploadcomplete: null, uploadprogress: null, error: function(e) {
        ...
        if (!this.params.width)
            this.params.width = e.offsetWidth;
        if (!this.params.height)
            this.params.height = e.offsetHeight;
        ...
        if (this.userMedia) {
            ...
            s.style.width = "" + this.params.dest_width + "px";
            s.style.height = "" + this.params.dest_height + "px";
            if (a != 1 || i != 1) {
                e.style.overflow = "hidden";
                s.style.webkitTransformOrigin = "0px 0px";
                s.style.mozTransformOrigin = "0px 0px";
                s.style.msTransformOrigin = "0px 0px";
                s.style.oTransformOrigin = "0px 0px";
                s.style.transformOrigin = "0px 0px";
                s.style.webkitTransform = "scaleX(" + a + ") scaleY(" + i + ")";
                s.style.mozTransform = "scaleX(" + a + ") scaleY(" + i + ")";
                s.style.msTransform = "scaleX(" + a + ") scaleY(" + i + ")";
                s.style.oTransform = "scaleX(" + a + ") scaleY(" + i + ")";
                s.style.transform = "scaleX(" + a + ") scaleY(" + i + ")"
            }
            e.appendChild(s);
            this.video = s;
            var r = this;
            navigator.getUserMedia({audio: false, video: true}, function(e) {
                s.src = window.URL.createObjectURL(e) || e;
                Webcam.stream = e;
                Webcam.loaded = true;
                Webcam.live = true;
                ...
                return r.dispatch("error", "Could not access webcam.")
            })
        } else {
            var o = document.createElement("div");
            o.innerHTML = this.getSWFHTML();
            e.appendChild(o)
        }
        if (this.params.crop_width && this.params.crop_height) {
            var h = Math.floor(this.params.crop_width * a);
            var n = Math.floor(this.params.crop_height * i);
            e.style.width = "" + h + "px";
            e.style.height = "" + n + "px";
            e.style.overflow = "hidden";
            e.scrollLeft = Math.floor(this.params.width / 2 - h / 2);
            e.scrollTop = Math.floor(this.params.height / 2 - n / 2)
        } else {
            e.style.width = "" + this.params.width + "px";
            e.style.height = "" + this.params.height + "px"
        }
    }, reset: function() {
        if (this.userMedia) {
            try {
        ...
        if (typeof this.hooks[e] == "undefined")
            throw"Event type not supported: " + e;
        this.hooks[e] = t
    }, dispatch: function() {
        var e = arguments[0].replace(/^on/i, "").toLowerCase();
        var t = Array.prototype.slice.call(arguments, 1);
        if (this.hooks[e]) {
            if (typeof this.hooks[e] == "function") {
                this.hooks[e].apply(this, t)
            } else if (typeof this.hooks[e] == "array") {
                this.hooks[e][0][this.hooks[e][1]].apply(this.hooks[e][0], t)
            } else if (window[this.hooks[e]]) {
                window[this.hooks[e]].apply(window, t)
            }
            return true
        }
        return false
        ...
        if (!this.swfURL) {
            ...
            for (var i = 0, s = a.length; i < s; i++) {
                var r = a[i].getAttribute("src");
                if (r && r.match(/\/webcam(\.min)?\.js/)) {
                    t = r.replace(/\/webcam(\.min)?\.js.*$/, "");
                    i = s
                }
            }
            if (t)
                this.swfURL = t + "/webcam.swf";
            else
        ...
        var o = "";
        for (var h in this.params) {
            if (o)
                o += "&";
            o += h + "=" + escape(this.params[h])
        }
        ...
        s.width = this.params.dest_width;
        s.height = this.params.dest_height;
        var r = s.getContext("2d");
        var o = function() {
            if (this.src && this.width && this.height) {
                r.drawImage(this, 0, 0, i.dest_width, i.dest_height)
            }
            if (i.crop_width && i.crop_height) {
                ...
                o.drawImage(s, Math.floor(i.dest_width / 2 - i.crop_width / 2), Math.floor(i.dest_height / 2 - i.crop_height / 2), i.crop_width, i.crop_height, 0, 0, i.crop_width, i.crop_height);
                r = o;
                s = a
            }
            if (t) {
            ...
            e(t ? null : s.toDataURL("image/" + i.image_format, i.jpeg_quality / 100), s, r)
        };
        if (this.userMedia) {
            r.drawImage(this.video, 0, 0, this.params.dest_width, this.params.dest_height);
            o()
        } else {
            var h = this.getMovie()._snap();
            var n = new Image;
            n.onload = o;
            n.src = "data:image/" + this.params.image_format + ";base64," + h
        }
        return null
